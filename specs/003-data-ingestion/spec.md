...
- **FR-003**: System MUST allow admins to map CSV columns to judge record fields (full name, court type, county name, state name, term start, term end, selection method, appointing authority, education, prior experience, political affiliation, source URL). State is specified once per import batch via a dropdown selector and MUST support non-standard column headers through a manual column mapping interface. If a State column is present in the CSV, the system MUST validate each row's state value against the dropdown-selected state (matching on name, abbreviation, or slug, case-insensitive) and flag mismatches as invalid rows.
